...
export const arrayHTTP: IObjectHeadingAnswers[] = [
  {
    ...
    answer: [
      "Интернет — это глобальная сеть взаимосвязанных компьютеров и устройств созданная для хранения и передачи информации. ",
  ...
  },
  {
    ...
    answer: [
      "Сервер — программа, которая хранит и обрабатывает ресурсы, обрабатывает сетевые запросы пользователей и обеспечивает их файлами для создания веб-страницы. Сервер может располагаться на одном или нескольких компьютерах; но даже в одном компьютере может быть несколько виртуальных серверов.",
  ...
  },
  {
    ...
    answer: [
      "Клиент — это программа, которая запрашивает у сервера доступ к ресурсам. Клиентом может быть любое устройство, способное делать запрос на сервер. Часто в роли клиента выступает веб-браузер, программы для отладки приложений или даже командная строка. Главная особенность клиента — он всегда инициирует запрос на доступ к ресурсам, для этого он использует API. ",
  ...
  {
    heading: "Виджет — это элемент интерфейса. ",
    answer: [
      "Виджет — это элемент интерфейса, который облегчает доступ к какой-либо информации. Например, на экран компьютера можно вывести виджет, который будет показывать последние новости или курс валют. ",
    ],
    isParagraph: true,
  ...
  },
  {
    ...
    answer: [
      "    HTTP — это протокол передачи гипертекста, который позволяет обмениваться данными в Интернете, например HTML-документами. Он создан для связи между веб-браузерами и веб-серверами. HTTP является протоколом клиент-серверного взаимодействия. ",
      "    HTTP представляет собой список правил, по которым компьютеры обмениваются данными в интернете. HTTP умеет передавать все возможные форматы файлов — например, видео, аудио, текст. Но при этом состоит только из текста. Его инициирует клиент, обрабатывает сервер и возвращает обратно клиенту. Между ними могут находиться прокси-серверы, которые занимаются дополнительными задачами — шифрованием данных, перераспределением нагрузки или кэшированием.",
  ...
  {
    heading: "Стартовая строка HTTP-ответа - строка статуса (Status line). ",
    answer: [
  ...
  },
  {
    ...
    answer: [
      "    1. Пользовательский интерфейс (user interface) — это все элементы браузера, с которыми может взаимодействовать пользователь. К интерфейсу относятся кнопки, поисковая строка, панель закладок, главная страница браузера, визуальный облик окна — всё, кроме содержимого сайтов. ",
      "",
      "    2. Движок браузера (browser engine) — это механизм, который переводит закодированную информацию с сервера в интерактивное изображение на экране. Работает в тесной связке с движком рендеринга. Chrome, «Яндекс Браузер», Opera, Microsoft Edge работают на движке Blink. Mozilla Firefox на собственном браузерном движке Quantum. Safari использует WebKit.",
      "",
      "    3. Движок рендеринга (rendering engine) — механизм для визуализации графики. ",
      "",
      "    4. Сетевая подсистема (networking) — обрабатывает сетевые запросы и отвечает за безопасность ",
      "",
      "    5. Подсистема поддержки пользовательского интерфейса (UI backend) отвечает за вывод основных компонентов интерфейса, таких как окна и элементы управления. ",
      "",
      "    6. JavaScript-движок (JavaScript engine) — выполняет JS-код сайтов и передаёт его в механизм рендеринга для отображения. ",
      "",
      "    7. Подсистема постоянного хранения данных (data persistence) — хранит пользовательские данные, такие как закладки, файлы cookie и настройки. ",
    ],
    isParagraph: true,
  },
  {
    heading:
      "Что такое CORS (Cross-Origin Resource Sharing, англ. «совместное использование ресурсов разных источников») ?",
    answer: [
