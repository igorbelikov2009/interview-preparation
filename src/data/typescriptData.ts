...
export const arrayTypescript: IObjectHeadingAnswers[] = [
  ...
  },
  {
    ...
    answer: [
      ...
      "- Any - тип данных в TypeScript, который описывает значения любого типа. Любой тип - во встроенных типах данных any является специальным типом данных, а также супертипом данных для всех типов данных. Если переменная объявлена с любым типом данных, то мы можем присвоить этой переменной значение любого типа.",
      "- Встроенные типы - это базовые типы тайпскрипта. Они включают в себя примитивные типы тайпскрипта: число, строка, логичексий тип, null, undefined, any, unknown, void, never. ",
      "- Определяемый пользователем тип. Пользователь также может определять свой собственный тип данных. Определяемые пользователем типы включают перечисления (enums), классы, интерфейсы, массивы и кортеж.",
  ...
  },
  {
    ...
    answer: [
      "Пользователь также может определять свой собственный тип данных. Определяемые пользователем типы включают перечисления (enums), классы, интерфейсы, массивы и кортеж.",
      "",
    ],
    isParagraphBefore: true,
  },
  ...
  },
  {
    ...
    answer: [
      "Any - это тип данных в TypeScript, который описывает значения любого типа. “any” используется, когда мы имеем дело со сторонними программами и ожидаем любую переменную, но мы не знаем точного типа переменной. Любой тип данных используется, потому что это помогает при регистрации и отказе от проверки типов во время компиляции.",
      "Синтаксис:",
      "let temp: any;",
      "temp = 'string';",
      "temp = 2;",
      "temp = true;",
      "Поскольку мы ввели “any” данных в temp, он ожидает все типы data-type и не выдает никаких ошибок. Это обеспечивает такую гибкость, что также позволяет компилятору не генерировать ошибку во время компиляции, если мы вызываем какой-либо метод для неизвестного типа, которому присвоен любой тип.",
      "--",
      "И мы убедили компилятор, что это “any” и имеет некоторую функцию для него, но мы не знаем, что это точно. Сейчас мы видели “any” данных, но поскольку TypeScript - это язык проверки типов, мы предоставляем переменную гибкость, но это вызывает некоторые проблемы, такие как вопросы безопасности, поэтому давайте обсудим, когда использовать любой тип данных.",
      "“any” используется только тогда, когда мы ничего не знаем о типе данных переменной и хотим получить динамическое содержимое из источника. Давайте посмотрим на некоторые ситуации, когда нам приходится использовать любой тип данных.",
      "--",
  ...
  },
  {
    ...
    answer: [
      "Unknown -  это аналог any, который также описывает значения любого типа. Однако, прежде чем работать со значением unknown, необходимо убедиться в его типизации (проверить) или переопределить его тип(<> as). ",
      "-*-",
      "Пример: чтобы работать со значением unknown, определяем его тип: ",
      ...
      "}",
      "-*-",
      "1. Особенности типа Unknown: ",
      "1.1. В TypeScript, чтобы работать со значением unknown, необходимо определить его тип.",
      "1.2. unknown присваивается самому себе или any.",
      ...
      "2.1. Описание с помощью типа unknown неизвестного значения, например возвращаемого из сетевого вызова:",
      "2.2. В утверждении типа",
      "*",
    ],
    isParagraphBefore: true,
  },
  {
    ...
    answer: [
      ...
      "- Вместо any.",
      "- Результаты JSON.parse (или любая другая десериализация в рантайме)",
  ...
  {
    heading: "Когда тип “unknown” нам не подойдёт?",
    answer: [
      "Единственный случай когда нам нужно использовать “any” вместо “unknown”:  ",
      " ",
      " ",
      " ",
      " ",
  ...
  },
  {
    ...
    answer: [
      "“Void” - это тип, который показывает, что функция ничего не возвращает.",
      "",
      "",
  ...
  },
  {
    ...
    answer: [
      "Never — это специальный тип, который обычно используется для определения типа значения в невозможной ситуации: default в свитче, который никогда не вызовется, или тип возвращаемого значения функции, которая не вернёт значение. https://purpleschool.ru/blog/typescript-never-type ",
      "Never находится в самом низу дерева иерархии типов, от которого не отходят никакие другие ветви, имеет два свойства:",
      "- переменной типа never можно присвоить только тип never и никакой другой.",
      ...
      "1. Для дополнительной проверки кампилятором, что какая-то ситуация реально не может произойти.",
      "2. Для того, чтобы сымитировать пустой элемент в union типе.",
      "-*-",
      "Данный тип очень тесно связан с функциями, и “never” говорит о том, что мы результата не получим.",
      "-*-",
      "Never - это юнион, у которого нету элементов. Представляет из себя на уровне типизации кейс, который не может произойти, так как юнион с нулями элементов у нас тоже не может существовать.",
      "Рассмотрим пример:",
      "type Test = number | string | never; ",
      "never у нас сразу исчезает, потому, что это просто пустой элемент - ничего.",
      "// ^? type Test = number | string;",
      "Never может быть присвоен пременной с любым типом: any, string, unknown, undefined и т.д.",
      "А в never мы не можем присвоить ничего, даже any. ",
      "Когда у нас появляется never?",
      "Never появляется тогда, когда мы доходим до кейса, который не может никогда произойти.",
      "-*-",
  ...
  {
    heading: "В TypeScript имеются следующие примитивные типы:",
    answer: [
      " - number : Числовой. Целые числа или числа с плавающей запятой.",
      " - string : Строчный. Последовательность символов Unicode, строки заключаются в одинарные или двойные кавычки.",
      ...
      " - Undefined: Неопределенный. Используется для всех неинициализированных переменных.",
      " - any: Любой. Описывает данные, тип которых может быть неизвестен на момент написания приложения. С ним проверка типов отключается, фактически разрешаются все типы.  ",
      " - unknown: Неизвестный. Описывает данные, тип которых может быть неизвестен на момент написания приложения. Аналогичен any, но безопаснее.",
      " - never — когда он определяется, выбрасывается ошибка. ",
      " - Void: Пустой. Отсутствие конкретного значения. Используется для функций, которые не возвращают значений.",
    ],
    isParagraphBefore: true,
  },
  {
    heading: "TypeScript: типы и дерево иерархии ",
    answer: [
      ...
      "Супертипы -  принимают значение любого типа и таким образом включают в себя все остальные типы. В TypeScript существует два типа, которые могут выступать как супертипы всех остальных типов — это any и unknown. Они занимают верхний уровень иерархии. ",
      "-*-",
  ...
  },
  {
    ...
    answer: [
      "object - представляет любой непримитивный тип, туда можно присвоить и function и array и объект.",
      "Object - используется для описания прототипа Object.prototype. Его не надо использовать. Туда можно присвоить всё, кроме undefined и null.  ",
  ...
  {
    heading: "Array используется для определения типов элементов массива.",
    answer: [
      "Есть два способа определения массивов: Array<T> и T[]",
      "1. let list: numder[] = [ 1, 2, 3 ]; // T[]",
      "2. let list: Array<number> = [ 1, 2, 3 ]; // Array<T>",
      "",
    ],
    isParagraphBefore: true,
  },
  ...
  {
    heading:
      "Как сделать так, чтобы переменные, функции, методы, классы, определенные в модуле, были доступны вне модуля?  ",
    answer: [
