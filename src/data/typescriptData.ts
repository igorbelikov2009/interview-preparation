...
export const linksTypescript: ILinkInfo[] = [
  ...
  {
    href: "https://runebook.dev/ru/docs/typescript/utility-types",
    title: "Utility Types - утилиты",
  },
  {
    href: "https://habr.com/ru/articles/711686/",
    title: "Искусство типизации: TypeScript Utility Types  - утилиты",
  },
  {
  ...
  {
    href: "https://digitrain.ru/articles/37765/",
    title: "Встроенные служебные типы в TypeScript - утилиты",
  },
  {
  ...
  {
    href: "https://habr.com/ru/companies/timeweb/articles/690726/",
    title: "Как работает утилита Parameters<typeof Type> TypeScript",
  },
  {
    href: "",
    title: "",
...
export const arrayTypescript: IObjectHeadingAnswers[] = [
  ...
  },
  {
    ...
    answer: [
      "Unknown -  это аналог any, который также описывает значения любого типа. Однако, прежде чем совершать какие-то операции над ним, для того, чтобы работать со значением unknown, необходимо определить его тип. ",
      "Unknown описывает данные, тип которых может быть неизвестен на момент написания приложения. Аналогичен any, но безопаснее. Unknown присваивается самому себе или any, а any — чему угодно.",
      "-*-",

      "Давайте разберемся, когда использовать неизвестные типы, а когда нет, с помощью следующих примеров.",

      "Пример 1. Мы можем присваивать Unknown значения любых типов и это не вызывает никаких ошибок.",
      "- let val: unknown;",
      ...
      " console.log(val); // undefined",
      "--",
      "Пример 2. Unknown присваивается самому себе или any,",
      "Как указано во введении, переменная неизвестного типа может быть присвоена только другой переменной неизвестного типа или переменной типа any. Unknown отображается как “undefined”.",
      "- let a: unknown;",
      ...
      "--",
      "Пример 3. В этом примере мы пытались присвоить Unknown переменной типа number, возникает ошибка.",
      "let unknown: unknown;",
      "let num: number = unknown; // Error",
      "console.log(num); // error TS2322: Тип «неизвестный» не может быть присвоен другому типу, только самому себе или any.",
      "--",
  ...
  },
  {
    ...
    answer: [
      ...
      "- Вместо any.",
      "- Результаты JSON.parse (или любая другая десиареализация в рантайме)",
  ...
  },
  {
    ...
    answer: [
      "Unknown -  это аналог any, который также описывает значения любого типа. Однако, прежде чем совершать какие-то операции над ним, для того, чтобы работать со значением unknown, необходимо определить его тип: проверить и переназначить(<> as) ",
      "-*-",
  ...
  },
  {
    ...
    answer: [
      ...
      "",
      "Never находится в самом низу дерева иерархии типов, от которого не отходят никакие другие ветви. ,это нижний тип в  с двумя свойствами:",
  ...
  {
    heading: "{ } – комплексный объект, которому можно определить свойства.",
    answer: [
      "{name: string, age: number} // объект с атрибутами имени и возраста.",
      "-*-",
      "let person = {name:'Kira', age:25};",
      "console.log(person.name); // Kira",
  ...
  },
  {
    ...
    answer: [
      ...
      "1. Awaited<T> - это специальный тип, который может быть использован для обозначения типа, который будет возвращен из асинхронной функции.",
      ...
      "16. OmitThisParameter<Type> - определяет функцию без типа this.",
      "17. ThisType<Type> - добавляет тип this к функции Type.",
      "18. Управление регистром - это утилитные типы для манипуляции строками, которые изменяют регистр строки в соответствии с их именем.",
      "-*-",
  ...
  },
  {
    ...
    answer: [
      "Readonly<T> - делает все свойства объекта типа T доступными только для чтения (readonly). Такие свойства являются иммутабельными (их значения нельзя изменить, переназначить). Используя эту утилиту, мы в некоторой степени замораживаем объект, чтобы предотвратить редактирование.",
      ...
      "};",
      "-",
      "Объявляем тип объекта с обязательными полями User:",
      "type User = {",
      "- name: string",
      "- password: string",
      "- addres: string",
      ...
      "}",
      "-",
      "Преобразуем тип User в тип ReadonlyUser с неизменными полями, доступными только чтения сделаем это через утилиту Readonly<T>",
      "-",
      "type Readonly<User> = {",
  ...
  },
  {
    ...
    answer: [
      "Pick<Type, Keys> - выбирает только указанные свойства Keys из исходного объектного типа Type. Эта утилита позволяет создать новый тип посредством извлечения из Type набора (множества) свойств Keys (Keys — строковый литерал или их объединение).",
  ...
  {
    heading: "15. Утилита Uppercase<StringType>.",
    answer: [
  ...
  {
    heading: "16. Утилита Lowercase<StringType>.",
    answer: [
  ...
  {
    heading: "17. Утилита Capitalize<StringType>.",
    answer: [
  ...
  {
    heading: "18. Утилита  Uncapitalize<StringType>.",
    answer: [
  ...
  {
    heading: "Дженерики в TypeScript",
    answer: [
      "В документации TypeScript приводится следующее определение: 'дженерики — это возможность создавать компоненты, работающие не только с одним, а с несколькими типами данных'.",
      "Основная идея состоит в том, что дженерики позволяют нам создавать некие повторно используемые компоненты, работающие с различными типами передаваемых им данных.",
      "Суть дженериков 👉 дженерики — переменные, через которые мы передаём тип.",
      "-*-",
