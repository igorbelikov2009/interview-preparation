...
export const arrayTypescript: IObjectHeadingAnswers[] = [
  ...
  },
  {
    ...
    answer: [
      "1.  Тип “void” показывает, что функция или метод ничего не возвращают, что у функции или метода отсутствует возвращаемое значение.",
  ...
  },
  {
    ...
    answer: [
      "1. Never — это тип, который указывает, что эти операции никогда не будут выполнены; эти значения никогда не встречаются; этих значений никогда не будет; эти функции никогда не вернут определённого значения и результата не получим.",
      "-*-",
      ...
      "3.1. Для дополнительной проверки кампилятором, что какая-то ситуация реально не может произойти.",
      "3.2. Для того, чтобы сымитировать пустой элемент в union типе.",
      "Рассмотрим пример:",
      ...
      "- переменной типа never можно присвоить только тип never и никакой другой, даже any.",
      "- never расширяет все типы и может быть присвоен пременной с любым типом: any, string, unknown, undefined и т.д.",
      "-*-",
  ...
  },
  {
    ...
    answer: [
      ...
      "3.1. В TypeScript, чтобы работать со значением unknown, необходимо проверить его тип или определить ему тип(<> as).",
      "3.2. unknown присваивается самому себе или any.",
      ...
      "4.1. Описание с помощью типа unknown неизвестного значения, например возвращаемого из сетевого вызова:",
      "4.2. В утверждении типа",
      "*",
    ],
    isParagraphBefore: true,
  },
