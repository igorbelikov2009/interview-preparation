...
export const arrayTypescript: IObjectHeadingAnswers[] = [
  ...
  {
    heading: "Что такое тип “Unknown” (неизвестный) тип? Когда нужно использовать?",
    answer: [
  ...
  {
    heading: "Как сделать все или одно (2, 3, 4...) свойства интерфейса необязательными?",
    answer: [
      "1. В TypeScript существует возможность объявлять необязательным свойством, которое может отсутствовать при инициализации. Оно отмечаются символом '?',",
      "interface ICircle {",
      " Radius: number;",
      " Color?: string;",
      "}",
      ...
      "--",
      "2. Partial<T> можно использовать partial, чтобы сделать все (все) свойства необязательными..",
  ...
  },
  {
    ...
    answer: [
      "Остаточные параметры позволяют передавать в функцию различное количество аргументов (ноль или более). Это полезно, когда вы не уверены, сколько параметров получит функция. Все аргументы после символа остатка ... будут храниться в массиве. Например:",
      ...
      "Остаточный параметр должен быть последним в определении параметра, и у вас может быть только 1 остаточный параметр для каждой функции.",
      "",
    ],
    isParagraphBefore: true,
  },
