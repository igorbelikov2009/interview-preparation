...
export const arrayTypescript: IObjectHeadingAnswers[] = [
  {
    ...
    answer: [
      "TypeScript — это язык программирования с открытым исходным кодом. Это надмножество языка JavaScript. Type-Script предназначен для разработки больших приложений. Он разработан и поддерживается Microsoft. TypeScript синтаксически повторяет JavaScript, но добавляет к нему больше возможностей. Это надмножество JavaScript. ",
      " ",
  ...
  },
  {
    ...
    answer: [
      ...
      "Статической типизацией называется контроль типов, который происходит на этапе компиляции.",
      "      *  Компиляция: это когда код, написанный на языке программирования, «переводят» в машинные команды целиком.",
      "Динамической типизацией называется контроль типов, который происходит на этапе выполнения программы.",
    ],
    isParagraphBefore: true,
  },
  {
  ...
  },
  {
    ...
    answer: [
      ...
      " ",
      "В TypeScript существует два типа данных: встроенные и пользовательские типы. ",

      "    1.1. Any - встроенный, специальный тип, который может использоваться для отключения проверки типов.",
      "    1.2. Встроенные, примитивные Number, String, Boolean, Symbol, BigInt, Null, Undefined, Void.",
      "    1.3. Встроенные, специальные типы Never, Unknown",
      "    1.4. Встроенные служебные типы в TypeScript - утилиты (TypeScript Utility Types).",
  ...
  {
    heading: "Примитивные типы Number, String, Boolean, Symbol, BigInt, Null, Undefined .",
    answer: [
      ...
      "4. Symbol символьный тип для создания глобально уникальных ссылок с помощью функции Symbol().",
      "5. BigInt (bigint) для представления очень больших целых чисел.",
      "6. Null - примитивный тип. Имеет явно присвоенное значение null, которое означает отсутствие значения.",
      "7. Undefined - переменная не определена. Она объявлена, но ей не присвоено какое-либо значение.",
    ],
    isParagraphBefore: true,
  },
  {
    heading: "Специальные примитивные типы Any, Unknown, Never, Void. ",
    answer: [
      "1. “Any” описывает значения любого типа. Использование 'any' равноценно отказу от проверки типов для переменной.",
      "2. Unknown - это аналог any, который также описывает значения любого типа. Однако, прежде чем работать со значением unknown, необходимо убедиться в его типизации (проверить) или переопределить его тип(<> as).",
      "3. Never - примитивный тип данных. Служит для указания того, что какие-либо операции никогда не будут выполнены.",
      "4. Void - отсутствие конкретного типа. Void явно указывает на то, что у функции или метода отсутствует возвращаемое значение. Оно есть, но оно undefined.",
      "",
    ],
    isParagraphBefore: true,
  },
  {
    ...
    answer: [
      ...
      " ",
      "      console.log(variableNull == null); // true ",
      "      console.log(variableUndefined == null); // true ",
      " ",
      "      console.log(variableNull === null); // true ",
      "      console.log(variableUndefined === null); // false ",
      " ",
  ...
  },
  {
    ...
    answer: [
      ...
      "    *  Строгое ‘===’ или не строгое равенство ‘==’. ",
      "    *  Строгое неравенство ‘!==’.  ",
      "    *  Оператор нулевого слияния ‘??’. ",
      " ",
      "1. Строгое ‘===’ или не строгое равенство ‘==’.  ",

      "      function getMessage(message: string | null | undefined) { ",
      ...
      "      console.log(getMessage(null)); // default message ",
      "      console.log(getMessage(undefined)); // default message ",
      "      console.log(getMessage('Ok')); // Ok ",
      " ",
  ...
  },
  {
    ...
    answer: [
      ...
      "          type A = NonNumber<string | number | boolean>;  ",
      " ",
      "          const str: A = 'Stroka'; ",
      "          const num: A =25 // Тип 'number' не может быть назначен для типа 'string'. ",
      "          const verity: A = true; ",
      "          const noVerity: A = false; ",
      " ",
      ...
      " ",
      "     3.4. Благодаря never мы можем исключить не нужные типы. Например, сделаем тип, который если у нас передано значение с number мы присваиваем never. Тем самым мы оставляем только нужный тип в типе А:",
      "          type NonNumber<T> = T extends number ? never : T; ",
      "          type A = NonNumber<string | boolean | number>; ",
      " ",
      "          let variable1: A = '7'; ",
      "          let variable2: A = true; ",
      "          let variable3: A = 7; // Тип 'number' не может быть назначен для типа 'string'.ts(2322) ",
      " ",
  ...
  {
    heading:
      "Тип “Array” или типизированный массив. Array<string>, Array<number>, Array<boolean> или string[], number[], boolean[].",
    answer: [
  ...
  },
  {
    ...
    answer: [
      ...
      "      let vasy: [string, number] = ['Vasy', 25];",
      " ",
      "      printUser(user); // Tom, 36;",
      "      printUser(vasy); // Vasy, 25",
      " ",
      ...
      "      let [ username, userage, isMarried ] = tom;   // деструктуризация",
      " ",
  ...
  },
  {
    ...
    answer: [
      "Тип пересечения — это тип, объединяющий несколько видов в один. Это позволяет комбинировать множество типов для создания одного типа со всеми необходимыми свойствами. Объект этого типа будет иметь элементы из всех заданных типов. Оператор '&' используется для создания типа пересечения. ",
      " ",
  ...
  },
  {
    ...
    answer: [
      "Псевдоним типа — это просто присвоение существующему типу другого имени.",
      " ",
      "В Typescript псевдонимы типов являются псевдонимами для реальных типов, они дают новое имя существующему типу. Их можно создавать для любого существующего типа в TypeScript. Псевдоним на самом деле не создает новый тип, вместо этого он дает этому типу новое имя. Псевдоним типа можно реализовать, расширить, если он представляет собой объектный тип или пересечение объектных типов. ",
      " ",
  ...
  {
    heading: "Что такое интерфейс? (interface)",
    answer: [
      "Интерфейс в typescript — это типы для объекта, структура или скелет объекта. Интерфейс — это синтаксис программирования, который определяет объект только с теми типами данных, которые он должен иметь.",
      " ",
  ...
  },
  {
    ...
    answer: [
      ...
      " ",
      "Пример 3.  ",
      "Объявим функцию getObjectValues(), которая создаст новый объект, выбрав из исходного определёные поля.",
      " ",
      "      const getObjectValues = <Obj, Key extends keyof Obj>(obj: Obj, array: Key[]) => { ",
      ...
      "      }; ",
      " ",
      "Инициализируем объект user, из которого в дальнейшем будем выбирать поля для нового объекта, ",
      " ",
      "      const user = { ",
      "            name: 'Dima',",
      "            age: 24,",
      "            channel: 'SIBERIA CAN COD - Frontend',",
      "      }; ",
      " ",
      "Создадим новый объект с помощью нашей функции: ",
      " ",
