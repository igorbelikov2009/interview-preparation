...
export const arrayTypescript: IObjectHeadingAnswers[] = [
  ...
  {
    heading: "Тип enum или перечисление. ",
    answer: [
      ...
      "-*-",
      "По умолчанию, перечисления являются числовыми, как в примере выше, и представляют собой числовые значения. Элементы Enum нумеруются, то есть, каждой именованной константе перечисления сопоставляется (назначается) числовое значение (свой индекс). По умолчанию, эта нумерация начинается с нуля и идёт по порядку.",
      "enum Env1 {",
      "- Development, // 0 ",
      "- Stading,     // 1 ",
      "- Production,  // 2 ",
      "} ",
      "Однако, этот порядок мы можем изменять, создавая свой индекс. Периодически такая необходимость может возникнуть. ",
      "То есть, мы сами можем назначить свойству значение: числовое, либо строковое. В этом случае его надо писать (присваивать). ",
      "enum Env2 {",
      "- Development = 2, //",
      "- Stading = 4, ",
      "- Production = 5, ",
      ...
      "} ",
      "-*- ",
      "Мы можем обратиться к свойству через точку или через квадратные скобки: ",
      "- console.log(Env3.Development); // development  ",
      "- console.log(Env3.Stading); // stading ",
      "- console.log(Env3.Production); // production ",
      "-*-",
      "Исползование енама оправдано только в том случае, если мы хотим использовать его как объект, например, где то доставать свойства, ",
      "-*- ",
    ],
    isParagraphBefore: true,
  },
  ...
  {
    heading: "Тип enum или перечисление. Строковые перечисления ",
    answer: [
