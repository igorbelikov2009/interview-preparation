...
exports.arrayJavascript = [
    ...
    },
    {
        ...
        answer: [
            "В браузере у нас есть движок, его v8 позволяет работать JavaScript. Движок интерпритирует наш код в байт-код. JavaScript — это однопоточный, интерпретируемый, скриптовый язык (сценариев) программирования с одиночным стеком вызовов. Это означает, что в один момент JavaScript может выполнять только одну операцию (обработать только один кусок кода). У нас есть возможность делать запросы, благодаря web-api. У браузера есть свой api, например, когда мы вызываем setTimeout, setInterval и прочее, браузер распределяет эти вызовы по очередям. То есть макро-таски, микро-такси, которые выполняют какой-то код JavaScripta, распределяются по очередям: макро-таски в одну очередь, микро-таски в другую очередь. Если встречаются промисы, ассихронные вызовы, то в другую очередь. То есть по-большому счёту, цикл событий - это система распределения вызова функций. ",
    ...
    },
    {
        ...
        answer: [
            ...
            "Микротаски имеют более высокий приоритет исполнения. Поэтому когда event loop прокручивается, ему не важно, сколько в очереди макротасок – он возьмёт микротаску, даже если она в очереди единственная, а макротасок 500.  ",
            "Все асинхронные операции происходят вне очереди. Call stack слепо исполняет те инструкции, которые ему передаёт event loop, координирующий работу всей программы.  ",
            "В очередь макро-тасок попадают SetTimeout, SetInterval, setImmediate, I/O, UI rendering",
            "В очередь микро-тасок попадают асинхронные операции: вызовы fetch(), axios, async-await, промисы, колбэки. ",
            "У микротасок высокий приоритет, у них своя очередь. Микротаски выполняются сразу, как только освободился стек. Другими словами: сначала выполнятся все микро-такси, только потом, когда очередь стане пустой, начнут выполняться макро-таски.",
            " ",
            " ",
            " ",
            " ",
    ...
    {
        heading: "Механизм исполнения асинхронной задачи браузера.",
        answer: [" ", " ", " ", " ", " ", " ", " ", " ", " ", " ", " ", " ", " "],
        isParagraph: true
    },
    {
    ...
    },
    {
        heading: "Операторы ~ и ~~  ",
        answer: [
            "Оператор ~ используется для изменения целых чисел (N+1), например: ",
            "console.log(~2);  // -3 ",
            "console.log(~1);  // -2 ",
            "console.log(~0);  // -1 ",
            "console.log(~-1); // 0 ",
            "-*-",
            "Оператор ~~ используется для округления числа с плавающей запятой в меньшую сторону: 3.1 становится 3, -1.1 становится -2, аналогичен Math.floor(). Например:",
            "console.log(~~2);    //  2 ",
            "console.log(~~2.4);  //  2 ",
            "console.log(~~3.9);  //  3 ",
            "console.log(~~-3.9); // -3",
            ".",
            "Самое главное в операторе ~~ :",
            "console.log(~~[undefined]); // 0. То есть оператор ~~ приводит [undefined] к нулю",
            ".",
            "Если вы хотите сделать это с помощью TypeScript, то используйте foo['undefined']",
            "Ключи:     ",
            "     foo[undefined],",
            "     foo['undefined'],",
            "     foo.undefined",
            "указывают на одно и то же свойство. ",
            "console.log(~~obj[undefined]) - Тип 'undefined' невозможно использовать как тип индекса.ts(2538); ",
            "console.log(~~obj['undefined']) - правильно, кампилятор пропускает эту запись ",
            "-*-",
        ],
        isParagraph: true
    },
