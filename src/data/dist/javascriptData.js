...
exports.arrayJavascript = [
    ...
    {
        heading: "Ссылочные типы данных. Поверхностное и вложенное клонирование (копирование) объектов, мутации и неизменяемость.",
        answer: [
            ...
            "      console.log(clone); // {name: 'Анна', age: 25, isAdmin: true }",
            " ",
            "3.  Вложенное клонирование. structuredClone(obj) позволяет клонировать бесконечно вложенные объекты и массивы, циклические ссылки, широкий спектр типов JavaScript, таких как: Date, Set, Map, Error, RegExp, ArrayBuffer, Blob, File, ImageData и многие другие.",
            "      let user = { ",
            "            name: 'John',",
            "            sizes: {",
            "                  height: 182,",
    ...
    },
    {
        ...
        answer: [
            "1. Промис — это ОБЪЕКТ JavaScript, который занимается обработкой асинхронного кода. Промис представляет результат успешного или неудачного завершения асинхронной операции. Промисы позволяют откладывать выполнение блоков кода до момента завершения асинхронной операции. ",
            "Асинхронная операция - это операция, которая выполняется независимо от внешнего кода, в котором она вызывается, и не блокирует выполнение вызываемого кода.  ",
            ...
            "После того как обещание установлено, результат можно получить через метод then. Обработать отказ можно либо через второй параметр в then, либо через catch. ",
            ...
            " ",
            "2. В сущности, промис - это возвращаемый объект, в который мы записываем два колбэка вместо того, чтобы передать их функции. Наши колбэки:",
            "    *  Успешный колбэк. ",
            "    *  Неуспешный колбэк. ",
            " ",
            "3. Главное в промисах. Применение промисов даёт несколько преимуществ. Давайте рассмотрим их. ",
            "      3.1. Промисы позволяют обрабатывать отложенные во времени события. Колбэки никогда не будут вызваны до завершения обработки текущего события в событийном цикле JavaScript. ",
            "      3.2. Колбэки, добавленные через .then даже после успешного или неудачного завершения асинхронной операции, будут также вызваны.",
            "      3.3. Несколько колбэков может быть добавлено вызовом .then нужное количество раз, и они будут выполняться независимо в порядке добавления. ",
            " ",
            "4. Цепочка вызовов. ",
            ...
            " ",
            "5. Промис может находиться в одном из трёх состояний:  ",
    ...
    },
    {
        ...
        answer: [
            "1. Промис — это ОБЪЕКТ JavaScript, который занимается обработкой асинхронного кода. Промис представляет результат успешного или неудачного завершения асинхронной операции. Промисы позволяют откладывать выполнение блоков кода до момента завершения асинхронной операции.",
            "Промисы улучшают читаемость и дают возможность создания цепочки вызовов промисов. Промис - это обещание предоставить результат позже. ",
            "Асинхронная операция - это некоторое действие, которое выполняется независимо от окружающего ее кода, в котором она вызывается, не блокирует выполнение вызываемого кода. ",
            " ",
            "2. В сущности, промис - это возвращаемый объект, в который мы записываем два колбэка вместо того, чтобы передать их функции. Наши колбэки:",
            " ",
            "Успешный колбэк: ",
            ...
            "            // В случае resolve: 'Успешно завершено с результатом Успех'",
            "      } ",
            " ",
            "Неуспешный колбэк: ",
            " ",
            ...
            "            // В случае reject: 'Завершено с ошибкой Ошибка'",
            "      } ",
            " ",
            "2.1. Старомодная функция, которая принимает два колбэка и вызывает один из них в зависимости от успешного или неудачного завершения операции: ",
            " ",
            "      function doSomethingOldStyle(successCallback, failureCallback) {   ",
            "            console.log('Готово.');",
            "            // Успех в половине случаев.",
            "            if (Math.random() > 0.5) {",
            "                  successCallback('Успех'); // Успешный колбэк",
            "            } else {",
            "                  failureCallback('Ошибка'); // Неуспешный колбэк",
            "            }",
            "      } ",
            " ",
            "Вызов старомодной функции: ",
            "      doSomethingOldStyle(successCallback, failureCallback); ",
            " ",
            "2.2. Cовременная функция возвращает промис, в который мы записываем наши колбэки: ",
            " ",
            ...
            "Это называется асинхронным вызовом функции. ",
            " ",
            "3. Главное в промисах. Применение промисов даёт несколько преимуществ. Давайте рассмотрим их. ",
            "      3.1. Промисы позволяют обрабатывать отложенные во времени события. Колбэки никогда не будут вызваны до завершения обработки текущего события в событийном цикле JavaScript. ",
            "      3.2. Колбэки, добавленные через .then даже после успешного или неудачного завершения асинхронной операции, будут также вызваны.",
            ...
            "Мы используем цепочку промисов, когда хотим выполнить последовательность промисов. Когда необходимо выполнить несколько асинхронных операций одна за другой, причём каждую следующую можно начать только при успешном завершении предыдущей, чтобы использовать результат её выполнения.",
            "Методы then() и catch() также могут возвращать новый промис, который может быть обработан цепочкой других then() в конце предыдущего метода then(). Вот в чём магия: функция then возвращает новый промис, отличающийся от первоначального:",
            " ",
            "              let promise = doSomething();",
            "          // let promise2 = promise.then(successCallback, failureCallback);",
            "или        let promise2 = doSomething().then(successCallback, failureCallback); ",
            " ",
            "Второй промис представляет не только завершение doSomething(), но и завершение функций successCallback или failureCallback, переданных нами, а они тоже могут быть асинхронными функциями, возвращающими промис. В этом случае все колбэки, добавленные к promise2 будут поставлены в очередь за промисом, возвращаемым successCallback или failureCallback. ",
            "По сути, каждый вызванный промис означает успешное завершение предыдущих шагов в цепочке. ",
            " ",
            "// Другой колбэк: ",
            "      function doSomethingElse(newResult: string) { ",
            "            console.log('doSomethingElse успешно завершено с результатом ' + newResult);",
            "      } ",
            "",
            "// Ещё один другой колбэк: ",
            "      function doThirdThing(thirdResult: void) { ",
            "            console.log('doThirdThing успешно завершено с результатом ' + thirdResult);",
            "      } ",
            " ",
            "              let promise = doSomething();",
            "          // let promise2 = promise.then(successCallback, failureCallback);",
            "или        let promise2 = doSomething().then(successCallback, failureCallback); ",
            " ",
            "3.1. В современных функциях мы записываем колбэки в возвращаемые промисы - формируем цепочку промисов: ",
            " ",
            "      doSomething() ",
            "            .then(function (result) {",
            "                  return doSomethingElse(result); // doSomethingElse успешно завершено с результатом Успех",
            "            })",
            "            .then(function (newResult) {",
            "                  return doThirdThing(newResult); // doThirdThing успешно завершено с результатом undefined",
            "            })",
            "            .then(function (finalResult) {",
            "                  console.log('Итоговый результат: ' + finalResult); // Итоговый результат: undefined",
            "            })",
            "            .catch(failureCallback); // Завершено с ошибкой Ошибка",
            " ",
            "3.2. Вот как это выражено с помощью стрелочных функций:",
            " ",
            "      doSomething() ",
            ...
            " ",
            "4.2. Промис может находиться в одном из трёх состояний:  ",
            "    *  pending — стартовое состояние, операция стартовала;  ",
            "    *  fulfilled — получен результат;",
            "    *  rejected — ошибка.",
        ],
    ...
    },
    {
        ...
        answer: [
            "1. Промис (Promise) используется для написания и обработки асинхронного кода. Это объект, который представляет итоги завершения асинхронной операции: успешный резальтат или неуспешный.",
            "",
            ...
            "2. Как пишется ",
            " ",
            "2.1. Объявляем асинхронную операцию, которую будем передавать в промис: ",
            "",
            "      const getData = (): Promise<any> => { ",
            "            return fetch('https://jsonplaceholder.typicode.com/todos/1');",
            "      }; ",
            " ",
            "2.2. Промис создаётся с помощью конструктора. ",
            ...
            "",
            "            const data = getData(); // вызываем асинхронную функцию, ",
            "                                                 // результат вызова сохраняем в переменной data.",
            "            resolve(data); // переводим промис в состояние fulfilled. ",
            "                                                 // Результатом выполнения будет объект data",
            "            console.log(data); // Promise {<pending>} ",
            "      }); ",
            " ",
            "2.3. Параметры промиса: ",
            ...
            "    *   finally()",
            " ",
            "4.1. Метод then() используют, чтобы выполнить код после успешного выполнения асинхронной операции.",
            ...
            " ",
            "4.2. Метод catch() используют, чтобы выполнить код в случае ошибки при выполнении асинхронной операции. ",
            ...
            " ",
            "4.3. Метод finally() используют, чтобы выполнить код при завершении асинхронной операции. Он будет выполнен вне зависимости от того, была ли операция успешной или завершилась ошибкой. ",
            ...
            "5. Цепочки методов.",
            " ",
            "5.1. Методы then(), catch() и finally() часто объединяют в цепочки вызовов, чтобы обработать и успешный, и ошибочный сценарии: ",
            " ",
            "      let isLoading = true; ",
            " ",
            ...
            "            }); ",
            " ",
            "5.2. В этом случае при успешном завершении операции мы выполним код из then(), при ошибке — код из catch(). Затем выполнится код из finally(). ",
            ...
            "                  return fetch(characterUrl) // Вызов fetch вернет промис, возвращаем его из колбэка, ",
            ...
            "            }) ",
            " ",
            "6. Обработка ошибок в цепочках методов",
            " ",
            "6.1.  fetch(...)",
            "                .then(...)",
            "                .then(...)",
            ...
            "                .catch(...)   ☝️ ",
            "",
            "Цепочки then() при обработке промисов могут быть очень большими. В примере выше цепочка состоит из шести then() и трёх catch(). Как в этом случае отработает catch()? ",
            "☝️ catch() обрабатывает ошибки от всех then() между текущим ☝️catch() и предыдущим ☝️catch(). ",
            " ",
            "6.2.  fetch(...)",
            "                .then(...)",
            "                .then(...)",
            ...
            "                .catch(...)   ☝️ ",
            "",
            "В примере выше наш ☝️ catch() — последний, а предыдущего нет, поэтому он будет обрабатывать все ошибки. ",
            " ",
            "6.3.  fetch(...)",
            "                .then(...)",
            "                .then(...)",
            ...
            " ",
            "7. Другие полезные методы промисов.",
            " ",
            "7.1. Promise.resolve()",
            ...
            "      }); ",
            " ",
            "8. Как создать асинхронную функцию с промисом.",
            "",
            "8.1. Создать функцию, которая будет выполнять асинхронную операцию:",
            "",
            "      function earnAllMoney() {};",
            " ",
            "8.2. Вернуть из функции свежесозданный промис:",
            "",
            "      function earnAllMoney() {",
            "            return new Promise(function (resolve, reject) { /* ... */ })",
            "      };",
            " ",
            "8.3. Аргументом в конструктор передать функцию, которая выполняет асинхронную операцию и переводит промис в состояние «успех» или «ошибка» в зависимости от результата:",
            "",
            "      function earnAllMoney() { ",
            "            return new Promise(function (resolve, reject) {",
            ...
            "      }; ",
            " ",
            "8.4. 💡 Если асинхронная операция работает через колбэки, то её стоит обернуть в промис, чтобы писать более читаемый код. ",
            "8.4.1. Рассмотрим пример с функцией getData, которая принимает два колбэка: первый вызывается при успехе, второй — при ошибке. Код этой функции может выглядеть так: ",
            " ",
            "      function getData(onSuccess: (arg0: number) => void, onError: (arg0: Error) => void) {",
            "            setTimeout(function () {",
            ...
            "      } ",
            " ",
            "8.4.2. Завернём её в промис: ",
            " ",
            "      function promiseifiedGetDate() { ",
            "            return new Promise(function (resolve, reject) {",
            ...
            "      } ",
            " ",
            "8.4.3. Теперь можно использовать методы then() и catch(): ",
            " ",
            "      promisifiedGetData() ",
            "            .then(function () {",
            "                  console.log('success')",
    ...
    },
    {
        ...
        answer: [
            ...
            "Метод then() всегда возвращает новый промис. ",
            " ",
            "2. Как пишется.",
            " ",
    ...
    },
    {
        ...
        answer: [
            ...
            "Метод catch()принимает один аргумент: ",
            "    *  onReject — функция-колбэк, которая будет вызвана при переходе промиса в состояние «ошибка» rejected. Функция имеет один параметр, в который передаётся информация об ошибке. ",
            ...
            "      }); ",
            " ",
            "3. Сatch() выполняет переданный ему колбэк в следующих случаях: ",
            "  3.1*  когда асинхронная операция вызывает функцию reject() внутри промиса.",
            ...
            "      }); ",
            " ",
            "  3.2* когда асинхронная операция выбрасывает ошибку с помощью throw.",
    ...
    },
    {
        ...
        answer: [
            ...
            " ",
            "1. Метод finally() используют для выполнения кода при завершении промиса. Код выполнится в любом случае, как при переходе промиса в состояние fulfilled, так и в rejected.",
            "Метод finally() принимает один аргумент: ",
            "    *  onDone — функция-колбэк, которая будет вызвана при завершении промиса.",
            "Метод finally() возвращает новый промис.",
            " ",
            "2. Как пишется.",
            " ",
            "getPasswords() — асинхронная функция, которая возвращает промис. ",
            ...
            "      let isLoading = true; ",
            " ",
            "4.1. Объявляем функцию checkMail(), в ней создаём промис. ",
            " ",
            "      function checkMail() { ",
            "            return new Promise((resolve, reject) => {",
            "                  if (Math.random() > 0.5) {",
            "                        resolve('Почта прибыла');",
            "                  } else {",
            "                        reject(new Error('Почта не удалось получить'));",
            "                  }",
            "            });",
            "      } ",
            " ",
            "4.2. Вызываем функцию checkMail(), используем методы промисов. ",
            " ",
            "      checkMail() ",
            "            .then((mail) => {",
            "                  console.log(mail);",
            "                  // в случае resolve: 'Почта прибыла'",
            "            })",
            "            .catch((err) => {",
            "                  console.error(err);",
            "                  // в случае reject: 'Error: Почта не удалось получить.'",
            "            })",
            "            .finally(() => {",
            "                  isLoading = false;",
            "                  // в любом случае: false",
            "                  console.log('Эксперимент завершен');",
            "                  // в любом случае:'Эксперимент завершен'",
            "            });",
        ],
        isParagraph: true
    },
    ...
    },
    {
        ...
        answer: [
            ...
            " ",
            "Async/await ",
            "1. Это синтаксический сахар, реализованный через обещания и корутины, который делает работу с асинхронными операциями более удобной. ",
            "Модификатор async ставится перед функцией и делает её асинхронной: ",
            "      async function asyncFunction () {",
            "            return '^_^'",
            "      } ",
            " ",
            "2. Результат асинхронной функции — это всегда обещание. Для удобства можно представлять асинхронную функцию как обычную, которая заворачивает свой результат в обещание: ",
            "      function asyncFunction () { ",
            "            return Promise.resovle('^_^')",
            "      } ",
            " ",
            "3. Результат асинхронной функции извлекается через then или через await: ",
    ...
    {
        heading: "Как скопировать массив",
        answer: [
            ...
            "      console.log(clonedArray); // [1, 2, 3] ",
            " ",
            "9. Используя методы JSON.parse() и JSON.stringify(): ",
            ...
            "      console.log(clonedArray); // [1, 2, 3] ",
            " ",
            "10. Используя метод Object.assign(): ",
