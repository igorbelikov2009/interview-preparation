...
exports.arrayJavascript = [
    ...
    },
    {
        ...
        answer: [
            ...
            "Компиляция. В этой фазе функциональные выражения и переменные, объявленные с помощью ключевого слова «var», со значением undefined поднимаются в самый верх текущей области видимости (как бы перемещаются в начало нашего кода. Это объясняет, почему мы можем вызывать функции до их объявления — прим. пер.). ",
    ...
    },
    {
        ...
        answer: [
            ...
            " ",
            "Если одна функция определена внутри другой, то внутренняя функция может получать доступ к переменной, объявленной во внешней функции, даже после того, как эта функция отработала. Это из-за того, что внутренняя функция является замыканием. ",
            " ",
            ...
            "peter(); // 'Peter' ",
            " ",
            "Здесь мы вызываем функцию person(), которая возвращает внутреннюю функцию displayName(), и сохраняем вызов этой функцию в переменной peter. Когда мы, после этого, вызываем функцию peter() (соответствующая переменная, хранит ссылку на функцию displayName()), в консоль выводится имя Peter. ",
            "При этом в функции displayName() нет переменной с именем name, но эта функция может получать доступ к такой переменной, объявленной во внешней функции, даже после того, как эта функция отработала. Это так из-за того, что функция displayName(), является замыканием. ",
        ],
        isParagraph: true
    },
    {
    ...
    },
    {
        ...
        answer: [
            ...
            "      }; ",
            " ",
            "3.1. Чтобы скопировать через Object.assign() нужно передать пустой объект",
            "",
            "      const adminCopy = Object.assign({}, admin); // const adminCopy: Admin ---> тип Admin",
            "      console.log(adminCopy); // {name: 'Анна', age: 21, isAdmin: true}",
            " ",
            "3.2.  Скопировать объект используя спред-синтаксис ... ",
            "",
            "      const anotherCopy = {  ",
            "            // --->  тип не Admin",
            "             ...admin,",
