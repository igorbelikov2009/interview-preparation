...
exports.arrayReact = [
    ...
    },
    {
        ...
        answer: [
            ...
            "Из-за JSX читаемость кода увеличивается, что помогает поддерживать более крупные приложения. ",
    ...
    },
    {
        ...
        answer: [
            ...
            "} ",
            "-- ",
            "Неконтролируемый компонент - это тот, который обладает внутренним стейтом. Данные формы обрабатываются самим DOM с помощью ref. Это немного больше похоже на традиционный HTML. ",
            "В целом, контролируемые компоненты считаются более предсказуемыми и их легче отлаживать, чем неконтролируемые компоненты. В большинстве (или во всех) случаях вам следует использовать управляемые компоненты. ",
            "--------------------------------------------- ",
    ...
    // },
    {
        ...
        answer: [
            ...
            "1. В React компонент с отслеживанием состояния, также известный как «умный» или «контейнерный» компонент, — это компонент, который поддерживает собственное внутреннее состояние, обычно с помощью хуков useState или this.state . Он также может управлять данными, которые передаются ему в качестве свойств от других компонентов, и может использовать методы жизненного цикла, такие как componentDidMount , для извлечения данных или выполнения других побочных эффектов. ",
    ...
    },
    {
        ...
        answer: [
            "Refs - cокращенно от References.",
            "Refs - это функция (или атрибут), предоставляет доступ к  DOM-узлам или React-элементам, созданным в методе рендеринга.",
            "Они используются в случаях, когда мы хотим изменить значение дочернего компонента, не используя props и все остальное. ",
            "Нужен для: ",
    ...
    },
    {
        ...
        answer: [
            "Благодаря вызову setState() React знает, что состояние изменилось. Когда состояние изменяется, компонент снова вызывает метод render().",
            "На основе нового состояния, определённые части приложения изменяются и строится новое дерево React элементов. Вычисляется, что должно отображаться на экране.",
            "Вызовы setState() являются асинхронными, поэтому время вывода отрисованного компонента будет включать в себя время обновлённия.",
    ...
    },
    {
        ...
        answer: [
            "Redux — это такая база данных в программе. Она хранит внутри себя данные приложения или, как говорят, состояние.  ",
            "Redux отвечает только за данные и никак не связан с браузером, DOM и фронтендом в целом. Теоретически Redux можно использовать даже на бекенде в Node.js. Redux, с точки зрения кода — это объект, внутри которого лежат данные. Он используется остальными частями приложения для их хранения, изменения и извлечения. ",
    ...
    },
    {
        ...
        answer: [
            ...
            "}, [ emitWidthColumn ]); ",
            "-- ",
            "// В jsx refs прикрепляются к элементам React через атрибут ref: ",
            " return ( ",
            " <div ref = { refColumn }> ",
            " - - <div> {title} </div>",
            " </div> ",
    ...
    },
    {
        ...
        answer: [
            "В React мемоизация — это метод, используемый для оптимизации производительности компонента за счет предотвращения ненужных повторных рендерингов. Он включает в себя кэширование результатов рендеринга компонента, поэтому, если входные данные (пропсы) для компонента не меняются, кэшированный результат можно использовать повторно вместо повторного вычисления результата. ",
